<?php
/*------------------------------------------------------------------------------
  sNews Version:	1.8.0 - Fork from Official Version (Reborn)
  CodeName:			Bootstrap
  Last Update		July 21, 2016 - 23:00 GMT+0
  Developpers: 		Rui Mendes
  Thanks to:		
...
function l($var) {
	...
	if (!$lang) {
		...
		# LEFT-MENU - BOOTSTRAP
		$lang['left_menu'] = 'pages,&ipages,&categories';
		# RIGHT-MENU - BOOTSTRAP
		$lang['right_menu'] = '';
		# MORE - BOOTSTRAP
		$lang['more1_menu'] = '';
		$lang['more2_menu'] = '';
...
function title() {
	...
	$lfeed = PHP_EOL.chr(9);
	# MOBILE FIRST
	echo '<meta charset="'.s('charset').'">'.$lfeed;
	echo '<meta http-equiv="X-UA-Compatible" content="IE=edge">'.$lfeed;
    echo '<meta name="viewport" content="width=device-width, initial-scale=1">'.$lfeed;
    echo '<meta name="description" content="'.(!empty($_DESCR) ? $_DESCR : s('website_description')).'">'.$lfeed;
    echo '<meta name="keywords" content="'.(!empty($_KEYW) ? $_KEYW : s('website_keywords')).'" />'.$lfeed;
    echo '<meta name="author" content="sNews 1.80">'.$lfeed;
	...
	# BOOTSTRAP CSS
	echo '<link href="bootstrap/css/bootstrap.min.css" rel="stylesheet">'.$lfeed;
	echo '<link href="bootstrap/css/bootstrap-theme.min.css" rel="stylesheet">'.$lfeed;
	echo '<link href="bootstrap/css/ie10-viewport-bug-workaround.css" rel="stylesheet">'.$lfeed;
    # Custom styles for this template
    echo '<link href="css/theme.css" rel="stylesheet">'.$lfeed;
    # HTML5 shim and Respond.js for IE8 support of HTML5 elements and media queries
    echo '<!--[if lt IE 9]>'.$lfeed.chr(9);
	    echo '<script src="https://oss.maxcdn.com/html5shiv/3.7.2/html5shiv.min.js"></script>'.$lfeed.chr(9);
    	echo '<script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>'.$lfeed;
    echo '<![endif]-->'.PHP_EOL;
    # JAVASCRIPT
...
// LOGIN/LOGOUT LINK - BOOTSTRAP
function login_link() {
	$link = '<a href="'._SITE;
	$login = '<li>'.$link;
	$must_login = 'login/" title="'.l('login').'">'.l('login');
	$login .= _ADMIN ? 'administration/" title="'.l('administration').'">'.l('administration').'</a>' : $must_login;
	$login .= _ADMIN ? ' </li><li><a href="'._SITE.'logout/" title="'.l('logout').'">'.l('logout') : '';
	$login .= '</a></li>';
	echo $login;
}
		...
		if ($page = isset($routes[$categorySEF]) ? $routes[$categorySEF] : null) {
			echo '<div class="panel panel-primary">';
				echo '<div class="panel-heading">';
					echo '<h3 class="panel-title">'.l($categorySEF).'</h3>';
				echo '</div>';
		...
		switch ($action) {
			...
			default :
				if (_ADMIN) {
					...
					echo '<div class="panel panel-primary">';
						echo '<div class="panel-heading">';
							echo '<h3 class="panel-title"><a href="http://snewscms.com/" title="sNews CMS">sNews</a> '.l('administration').'</h3>';
						echo '</div>';
						echo '<div class="panel-body">';
							switch ($action) {
...
/* ********************************************************************************************************* */
//									BOOTSTRAP NEW FUNCTIONS
/* ********************************************************************************************************* */
// BOOTSTRAP MENU LEFT
function bootstrap_menu() {
	$left_menu = explode(',', l('left_menu'));
	$right_menu = explode(',', l('right_menu'));
	echo '<nav class="navbar navbar-inverse navbar-fixed-top">';
		echo '<div class="container"><div class="navbar-header">';
			echo '<button type="button" class="navbar-toggle collapsed" 
				...
				echo '<span class="icon-bar"></span>';
			echo '</button>';
	    	echo '<a class="navbar-brand" href="'._SITE.'">'.s('website_title').'</a>';
		echo '</div>';
		echo '<div id="navbar" class="navbar-collapse collapse">';
			echo '<ul class="nav navbar-nav">';
			for ($i = 0; $i < count($left_menu); $i++) {
				if (function_exists($left_menu[$i])) {
					$func = $left_menu[$i];
					$func();
				} else if (substr($left_menu[$i], 0, 1) == '&') {
				 	$name = substr($left_menu[$i], 1);
				 	if (function_exists($name)) {
				 		echo '<li class="dropdown">';
							echo '<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" 
								aria-expanded="false">'.l($name).'<span class="caret"></span></a>';
			...
			echo '<li><a href="'._SITE.'searching/">Search</a></li>';
				login_link();
			echo '</ul>';
		echo '</div>';
	echo '</div></nav>';
